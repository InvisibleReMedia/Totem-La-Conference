			...
			><script
				...
				function sendQuestion(name) {
					...
					if (d.value.length > 1) {
						...
						fetch('../addQuestion.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; } ?>', {
							method : 'POST',
							body : data
						}).then(res => res.text())
							.then(text => {
				...
				function applaud() {
					fetch('../applaud.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>')
					...
					for(const u of json) {
						if (u.type == "QUESTION") {
							...
							await fetch('../isAutomaticModeration.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>')
								.then(res => res.text())
								.then(async text => {
									if (parseInt(text) === 1) {await fetch('../getQuestion.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>&id=' + u.id)
								.then(res => res.text())
								.then(text => {
							...
							if (u.infos.moderated == 0) {
									if (x.length > 0)
										x[0].parentNode.removeChild(x[0])
								} else {await fetch('../getQuestion.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>&id=' + u.id)
								.then(res => res.text())
								.then(text => {
									let x = document.getElementsByName('aQuestion')
									x = Array.prototype.slice.call(x).filter(a => a.dataset.evqust == u.id)
