<?php	include('../logging.php');
	...
	getAuthentName($cnx, $user, $greetings, $firstName, $lastName);
		...
		$sql = "SELECT JSON_EXTRACT(e.infos, \"$.pages[4]\") AS numPage FROM events AS e WHERE e.id = " . $event;
			if ($fetch = $cnx->query($sql)) {
				...
				$relatedEvent = $result['numPage'];
					...
					logging($cnx, "ERROR", "error query - reason:" . $cnx->error, "events", "rever.php");
			}
		...
		><head
			><meta charset="utf-8"><meta name="author" content="Business Forward Technology (business.forward.technology@gmail.com)"><meta name="description" content="Web application pour une interactivité à fort impact sociétal. Notre objectif est de répondre à vos besoins en interactivité digitale et contribuer à votre politique RSE en matière d'inclusion des personnes en situation de handicap."><meta name="generator" content="talenha - a programming language"><meta name="viewport" content="width=device-width, initial-scale=1.0"><link rel="preload" as="font" type="font/ttf" crossorigin="" href="../fonts/Open-Sans/OpenSans-Regular.ttf"/><link rel="icon" type="image/png" href="../favicon.png"/><title
				>Rêver - Conférence HANDIGO</title
		...
		><body
				...
				><div
					...
					><div
						...
						><span
							...
							><strong
								>Input :</strong
							>Sois le manager que tu rêverais avoir.</span
						><br/><span
							style="height:10pt"
							></span
						><br/><u
							>Jeu de rôles :</u
						>dans votre équipe une personne a un trouble bipolaire et ne s’est pas déclarée administrativement en tant que telle.  <br/><u
							>Question :</u
						>comment détecter les signaux de mal-être au travail ?  <br/><span
							style="height:10pt"
							></span
						><br/><span
							style="background-color:blue;color:white"
							>Réponses des bonnes pratiques par les étudiants via la web application</span
						><br/><span
							style="height:10pt"
							></span
						><br/><u
							>Objectif pédagogique :</u
						>définir les conditions d’accueil d’informations de la personne en situation de handicap.  <br/><u
							>Conclusion :</u
						>le manager ( happy chief officer ) favorise le bien-être de tous ses collaborateurs et permet aux personnes en situation de handicap d’oser se déclarer. Ce qui rend l’entreprise handi accueillante et diminue la contribution AGEFIPH. Favoriser la déclaration du handicap, une action win win !</div
					><div
						style="display:inline-flex;height:30px;width:100%;width:320px"
						><div
							style="width:50%"
							><input type="button" value="Source participant" onclick="document.location = '.php?source=0'"/></div
						><div
							style="width:50%"
							><input type="button" value="Source client" onclick="document.location = '.php?source=1'"/></div
						><div
							style="width:%"
					...
					><div
						...
						><div
							style="height:100%"
							><div
								style="display:inline-flex;height:100%;width:100%;height:auto"
								><div
									style="width:1%"
									>&nbsp;</div
								><div
									...
									><div
										id="questions"
										><?php $sql = "SELECT e.id AS id, q.content AS content, e.likes AS likes, e.dislikes AS dislikes, e.moderation AS moderation, e.selected AS selected, e.source AS source, e.id_question AS idQuestion, e.id_response AS myResponse, e.responseNumber AS responseNumber, (SELECT r.id FROM questionresponses AS r WHERE r.question = e.id_question AND r.id_event = e.id_event) AS response, (SELECT q1.response1 FROM questionresponses AS q1 WHERE q1.question = e.id_question AND q1.id_event = e.id_event) AS response1, (SELECT q2.response2 FROM questionresponses AS q2 WHERE q2.question = e.id_question AND q2.id_event = e.id_event) AS response2, (SELECT q3.response3 FROM questionresponses AS q3 WHERE q3.question = e.id_question AND q3.id_event = e.id_event) AS response3, NOW() AS date FROM questions AS q, evqust AS e WHERE e.id_event = " . (isset($relatedEvent) ? $relatedEvent : $event) . " AND e.id_question = q.id AND e.suppressed = 0 AND e.moderation = 1 AND e.source >= " . $source . " ORDER BY e.source DESC, e.likes DESC";
												if ($fetch = $cnx->query($sql)) {
														...
														logging($cnx, "ERROR", "error query - reason:" . $cnx->error, "", "/");
												}
												?></div
									></div
								><div
			...
			><script
				...
				function removeQuestion(id) {
					fetch('../removeQuestion.php?session=<?php echo $session;if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>&id=' + id)
				}

				function selectQuestion(id) {
					fetch('../selectQuestion.php?session=<?php echo $session;if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>&id=' + id)
				}
				 var likes = []
				...
				function sendLikes() {
					...
					for(const u of likes) {
						fetch('../addLikes.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>&id=' + u.id + '&likes=' + u.likes)
				    likes = []
					}
				}
				...
				function sendResponse(id, resp, num, source) {
					...
					fetch('../answerQuestion.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; } ?>', {
						method : 'POST',
						body : data
				...
				 		sortingTimer = false
						...
						for(let index = 1; index < l.length; ++index) {
							if (parseInt(previous.innerText) < parseInt(l[index].innerText) && parseInt(previous.dataset.source) < parseInt(l[index].dataset.source)) {
								redo = true
								++counter
				...
				async function getReview() {
					await fetch('../review.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; } ?>&last_date=' + last_date)
						.then(res => res.json())
				...
				async function processReview(json) {
					for(const u of json) {
						if (u.type == "QUESTION") {
							...
							await fetch('../isAutomaticModeration.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>')
								.then(res => res.text())
								.then(async text => {
									if (parseInt(text) === 1 && u.infos.source >= <?php echo $source?>) {await fetch('../getQuestionAnimateur.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>&id=' + u.id)
								.then(res => res.text())
								.then(text => {
							...
							if (u.infos.moderated == 0) {
									if (x.length > 0)
										x[0].parentNode.removeChild(x[0])
								} else {if (u.infos.source >= <?php echo $source?>) { await fetch('../getQuestionAnimateur.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>&id=' + u.id)
								.then(res => res.text())
								.then(text => {
									let x = document.getElementsByName('aQuestion')
									x = Array.prototype.slice.call(x).filter(a => a.dataset.evqust == u.id)
							...
							x = Array.prototype.slice.call(x).filter(a => a.dataset.evqust == evqust);
							if (x.length > 0) {
								await fetch('../getQuestionContent.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>&id=' + u.id)
								.then(res => res.text())
								.then(text => {
									x[0].innerText = text
								})
							...
							await fetch('../getQuestionAnimateur.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>&id=' + u.id)
								.then(res => res.text())
								.then(text => {
									let x = document.getElementsByName('aQuestion')
									x = Array.prototype.slice.call(x).filter(a => a.dataset.evqust == u.id)
