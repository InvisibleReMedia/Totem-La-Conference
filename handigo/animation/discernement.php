<?php	include('../logging.php');
	...
	getAuthentName($cnx, $user, $greetings, $firstName, $lastName);
		...
		$sql = "SELECT JSON_EXTRACT(e.infos, \"$.pages[0]\") AS numPage FROM events AS e WHERE e.id = " . $event;
			if ($fetch = $cnx->query($sql)) {
				...
				$relatedEvent = $result['numPage'];
					...
					logging($cnx, "ERROR", "error query - reason:" . $cnx->error, "events", "discernement.php");
			}
		...
		><head
			><meta charset="utf-8"><meta name="author" content="Business Forward Technology (business.forward.technology@gmail.com)"><meta name="description" content="Web application pour une interactivité à fort impact sociétal. Notre objectif est de répondre à vos besoins en interactivité digitale et contribuer à votre politique RSE en matière d'inclusion des personnes en situation de handicap."><meta name="generator" content="talenha - a programming language"><meta name="viewport" content="width=device-width, initial-scale=1.0"><link rel="preload" as="font" type="font/ttf" crossorigin="" href="../fonts/Open-Sans/OpenSans-Regular.ttf"/><link rel="icon" type="image/png" href="../favicon.png"/><title
				>Discernement - Conférence HANDIGO</title
		...
		><body
			...
			><div
				...
				><div
					...
					><div
						...
						>En un mot, c'est quoi le handicap pour vous ?  <br/>On range les mots par catégorie (avantage ou inconvénient).  <br/><u
							>Objectif pédagogique :</u
						>A chaque handicap sa compensation.  <br/><span
							style="background-color:blue;color:white"
							>Collecte des réponses via la web app handigo.ihacom.fr</span
						></div
					><div
			...
			><script
				...
				function sendLikes() {
					...
					for(const u of dislikes) {
						fetch('../addLikes.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>&id=' + u.id + '&likes=' + u.dislikes)
					}
					dislikes = []
				...
				function addDisLikes(id) {
					let v = dislikes.find(a => a.id == id)
					if (v) {
				 v.dislikes = v.dislikes - 1
					} else {
				 dislikes.push({ "id" : id, "dislikes" : -1 })
					}
				}
					...
					for(const u of json) {
						if (u.type == "QUESTION") {
							...
							await fetch('../isAutomaticModeration.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>')
								.then(res => res.text())
								.then(async text => {
									if (parseInt(text) === 1 && u.infos.source >= <?php echo $source?>) {await fetch('../getOpinionAnimateur.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>&id=' + u.id)
								.then(res => res.text())
								.then(text => {
							...
							x = document.getElementsByName('dislikes');
							evqust = u.id;
							x = Array.prototype.slice.call(x).filter(a => a.dataset.evqust == evqust);
							if (x.length > 0) {
								if (parseInt(x[0].innerText) < u.infos.totalDisLikes) {
									x[0].innerText = u.infos.totalDisLikes;
								}
							}
							...
							if (u.infos.moderated == 0) {
									if (x.length > 0)
										x[0].parentNode.removeChild(x[0])
								} else {if (u.infos.source >= <?php echo $source?>) { await fetch('../getOpinionAnimateur.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>&id=' + u.id)
								.then(res => res.text())
								.then(text => {
									let x = document.getElementsByName('aQuestion')
									x = Array.prototype.slice.call(x).filter(a => a.dataset.evqust == u.id)
							...
							await fetch('../getOpinionAnimateur.php?session=<?php echo $session; if (isset($relatedEvent)) { echo "&relatedEvent=" . $relatedEvent; }?>&id=' + u.id)
								.then(res => res.text())
								.then(text => {
									let x = document.getElementsByName('aQuestion')
									x = Array.prototype.slice.call(x).filter(a => a.dataset.evqust == u.id)
