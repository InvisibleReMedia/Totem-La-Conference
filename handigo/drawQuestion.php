<div
	...
	><img class="heart" tabindex="-1" draggable="false" data-evqust="<?php echo $id?>" src="../images/heart.png" alt="Likez"/><span
		...
		data-evqust="<?php echo $id?>"
		><?php echo $likes?></span
	><?php if ((isset($response1) || isset($response2) || isset($response3)) && !isset($myResponse)) { echo '<br/><div style="display:inline;float:right">'; if (isset($response1)) { echo '<span class="response" onclick="sendResponse(' . $idQuestion . ', ' . $response . ',1,0)">' . htmlspecialchars($response1) . '</span>'; } if (isset($response2)) { echo '<span class="response" onclick="sendResponse(' . $idQuestion . ', ' . $response . ',2,0)">' . htmlspecialchars($response2) . '</span>'; } if (isset($response3)) { echo '<span class="response" onclick="sendResponse(' . $idQuestion . ', ' . $response . ',3,0)">' . htmlspecialchars($response3) . '</span>'; } echo "</div>"; } ?></div
>
