<?php	include('../logging.php');
	...
	getAuthentName($cnx, $user, $greetings, $firstName, $lastName);
		...
		lang="fr"
		><head
			><meta charset="utf-8"><meta name="author" content="Business Forward Technology (business.forward.technology@gmail.com)"><meta name="description" content="Sessions - Administration TOTEM La conférence"><meta name="generator" content="talenha - a programming language"><meta name="viewport" content="width=device-width, initial-scale=1.0"><link rel="preload" as="font" type="font/ttf" crossorigin="" href="../fonts/Open-Sans/OpenSans-Regular.ttf"/><link rel="icon" type="image/png" href="../favicon.png"/><title
				>Sessions - Administration - TOTEM La conférence</title
			><link href="../css/model1.css" rel="stylesheet"/><link rel="stylesheet" href="../css/-horizontal-c0-320px.css" media="all and (min-width:320px) and (orientation:landscape)"/><link rel="stylesheet" href="../css/-horizontal-c0-700px.css" media="all and (min-width:700px) and (orientation:landscape)"/><link rel="stylesheet" href="../css/-horizontal-c0-1000px.css" media="all and (min-width:1000px) and (orientation:landscape)"/><link rel="stylesheet" href="../css/-vertical-c0-320px.css" media="all and (min-width:320px) and (orientation:portrait)"/><link rel="stylesheet" href="../css/-vertical-c0-700px.css" media="all and (min-width:700px) and (orientation:portrait)"/><link rel="stylesheet" href="../css/-vertical-c0-1000px.css" media="all and (min-width:1000px) and (orientation:portrait)"/><style
				>
					.connectionName {
								...
								><div
									...
									><div
										...
										><?php
											...
											?><a
											href="sessions.php?&option=<?php echo $option?>&num_page=<?php echo $num_page?>&lines_per_page=<?php echo $lines_per_page?>"
											>Actualiser</a
										><form
											method="POST"
											action="sessions.php?&option=<?php echo $option?>&num_page=<?php echo $num_page?>&lines_per_page=<?php echo $lines_per_page?>"
											><div
												id="newRecord"
												class="group hidden"
												><div
													...
													><div
														><div
															...
															><table
																...
																></table
															></div
														><input type="text" style="width:140px" name="date_start" data-ref="0"/><img src="date.png" style="position:relative;left:10px;top:5px;max-height:15px;cursor:pointer" onclick="loadCalendar('date_start',0);openCalendar(0)"/></div
													></div
												><div
													><div
														><label
															>Date de déconnexion</label
														></div
													><div
														><div
															class="hidden"
															...
															><table
																...
																></table
															></div
														><input type="text" style="width:140px" name="date_end" data-ref="0"/><img src="date.png" style="position:relative;left:10px;top:5px;max-height:15px;cursor:pointer" onclick="loadCalendar('date_end',0);openCalendar(0)"/></div
													></div
												><div
													><div
														><label
												...
												><div
													><div
														...
														></div
													><div
														><input type="text" name="id_event"/></div
													></div
												><div
													><div
										...
										><?php
											if (!empty($_POST)) {
												...
												if (array_key_exists('id_event', $_POST)) {
											...
											if ($postEnabled) {
												if ($insert) {
													$sql = "INSERT INTO sessions(date_start, date_end, id_user, id_event) VALUES (?, ?, ?, ?)";
												} else {
													$sql = "UPDATE sessions SET date_start = ?, date_end = ?, id_user = ?, id_event = ? WHERE session = ?";
												}
												if ($stmt = $cnx->prepare($sql)) {
													if ($insert) {
														$stmt->bind_param("ssii", $date_start, $date_end, $id_user, $id_event);
													} else {
														$stmt->bind_param("ssiis", $date_start, $date_end, $id_user, $id_event, $session);
													}
													if (!$stmt->execute()) {
														echo $stmt->error;
													}
												...
												if ($query = $cnx->query($sql)) {
													...
													$sql = "SELECT session, date_start, date_end, id_user, id_event FROM sessions" . " LIMIT " . strval($lines_per_page) . " OFFSET " . strval($skip_lines);
											...
											><thead
												><tr
													...
													><td
														style="width:10%"
														>Date de connexion</td
													><td
														style="width:10%"
														>Date de déconnexion</td
													><td
														style="width:5%"
														>Utilisateur</td
													><td
											...
											><tbody
												><!--make a dynamic loop of values--><?php
													...
													><td
														...
														></td
													><td
														><?php echo htmlspecialchars($row['session']) ?></td
													><td
														><?php echo htmlspecialchars($row['date_start']) ?></td
													><td
														><?php echo htmlspecialchars($row['date_end']) ?></td
													><td
														><?php echo htmlspecialchars($row['id_user']) ?></td
													><td
														><?php echo htmlspecialchars($row['id_event']) ?></td
													></tr
												><tr
													...
													><td
														...
														><form
															...
															><div
																class="group"
																><input type="hidden" name="session" value="<?php echo htmlspecialchars($row['session']) ?>"/><div
																	><div
																		><label
																			>Date de connexion</label
																		></div
																			...
																			><table
																					...
																					><td
																						...
																						><b
																							><span
																								...
																								onmouseenter="this.style.color='red'"
																								onmouseleave="this.style.color='black'"
																								onclick="earlyYear('cyear', <?php echo $row['session']?>)"
																								>&lt;</span
																							></b
																						></td
																					><td
																					...
																					><td
																						...
																						><b
																							><span
																								...
																								onmouseenter="this.style.color='red'"
																								onmouseleave="this.style.color='black'"
																								onclick="laterYear('cyear', <?php echo $row['session']?>)"
																								>&gt;</span
																							></b
																						></td
																					></tr
																					...
																					><td
																						...
																						><b
																							><span
																								...
																								onmouseenter="this.style.color='red'"
																								onmouseleave="this.style.color='black'"
																								onclick="earlyMonth('cmonth', <?php echo $row['session']?>)"
																								>&lt;</span
																							></b
																						></td
																					><td
																					...
																					><td
																						...
																						><b
																							><span
																								...
																								onmouseenter="this.style.color='red'"
																								onmouseleave="this.style.color='black'"
																								onclick="laterMonth('cmonth', <?php echo $row['session']?>)"
																								>&gt;</span
																							></b
																						></td
																					></tr
																				...
																				><tr
																					><td
																						colspan="3"
																						style="margin:0 auto"
																						><input type="button" value="Fermer" onclick="closeCalendar(<?php echo $row['session']?>)"/></td
																					></tr
																				></table
																			></div
																		><input type="text" style="width:140px" name="date_start" value="<?php echo htmlspecialchars($row['date_start']) ?>" data-ref="<?php echo $row['session']?>"/><img src="date.png" style="position:relative;left:10px;top:5px;max-height:15px;cursor:pointer" onclick="loadCalendar('date_start',<?php echo $row['session']?>);openCalendar(<?php echo $row['session']?>)"/></div
																	></div
																><div
																	><div
																		><label
																	...
																	><div
																		><div
																			...
																			><table
																				...
																				></table
																			></div
																		><input type="text" style="width:140px" name="date_end" value="<?php echo htmlspecialchars($row['date_end']) ?>" data-ref="<?php echo $row['session']?>"/><img src="date.png" style="position:relative;left:10px;top:5px;max-height:15px;cursor:pointer" onclick="loadCalendar('date_end',<?php echo $row['session']?>);openCalendar(<?php echo $row['session']?>)"/></div
																	></div
																><div
																	><div
																		><label
																...
																><div
																	><div
																		...
																		></div
																	><div
																		><input type="text" name="id_event" value="<?php echo htmlspecialchars($row['id_event']) ?>"/></div
																	></div
																><div
																	><div
