								...
								><div
									...
									><div
										...
										><?php
											...
											?><a
											href="questions-evqust.php?&option=<?php echo $option?>&num_page=<?php echo $num_page?>&lines_per_page=<?php echo $lines_per_page?>"
											>Actualiser</a
										><?php
											if (!empty($_POST)) {
												$postEnabled = true;
											...
											$sql = "SELECT COUNT(*) FROM questions as q, evqust as e WHERE q.id = e.id_question";
											if ($query = $cnx->query($sql)) {
												...
												$sql = "SELECT e.id, e.date, e.id_event, e.id_question, e.id_user, q.content FROM questions as q, evqust as e WHERE q.id = e.id_question" . " LIMIT " . strval($lines_per_page) . " OFFSET " . strval($skip_lines);
											...
											><tbody
												><!--make a dynamic loop of values--><?php
													...
													><td
														><?php echo htmlspecialchars($row['content']) ?></td
													></tr
												><?php
																	++$num_line;
																	}
