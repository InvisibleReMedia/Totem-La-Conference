								...
								><div
									...
									><div
										...
										><?php
											...
											?><a
											href="likes.php?&option=<?php echo $option?>&num_page=<?php echo $num_page?>&lines_per_page=<?php echo $lines_per_page?>"
											>Actualiser</a
										><form
											method="POST"
											action="likes.php?&option=<?php echo $option?>&num_page=<?php echo $num_page?>&lines_per_page=<?php echo $lines_per_page?>"
											><div
												id="newRecord"
												class="group hidden"
												><div
										...
										><?php
											...
											if ($postEnabled) {
												if ($insert) {
													$sql = "INSERT INTO likes(id_evqust, id_user, likes, suppressed) VALUES (?, ?, ?, ?)";
												} else {
													$sql = "UPDATE likes SET id_evqust = ?, id_user = ?, likes = ?, suppressed = ? WHERE id = ?";
												}
												if ($stmt = $cnx->prepare($sql)) {
													if ($insert) {
														$stmt->bind_param("iiii", $id_evqust, $id_user, $likes, $suppressed);
													} else {
														$stmt->bind_param("iiiii", $id_evqust, $id_user, $likes, $suppressed, $id);
													}
													if (!$stmt->execute()) {
														echo $stmt->error;
													}
												...
												if ($query = $cnx->query($sql)) {
													...
													$sql = "SELECT id, id_evqust, id_user, likes, suppressed FROM likes" . " LIMIT " . strval($lines_per_page) . " OFFSET " . strval($skip_lines);
