								...
								><div
									...
									><div
										...
										><?php
											...
											?><a
											href="questions.php?&option=<?php echo $option?>&num_page=<?php echo $num_page?>&lines_per_page=<?php echo $lines_per_page?>"
											>Actualiser</a
										><form
											method="POST"
											action="questions.php?&option=<?php echo $option?>&num_page=<?php echo $num_page?>&lines_per_page=<?php echo $lines_per_page?>"
											><div
												id="newRecord"
												class="group hidden"
												><div
													><div
														...
														></div
													><div
														><input type="text" name="content"/></div
													></div
												><div
													><div
														><label
														...
														></div
													><div
														><input type="text" name="readOnly"/></div
													></div
												><div
													><div
										...
										><?php
											...
											if ($postEnabled) {
												if ($insert) {
													$sql = "INSERT INTO questions(content, readOnly) VALUES (?, ?)";
												} else {
													$sql = "UPDATE questions SET content = ?, readOnly = ? WHERE id = ?";
												}
												if ($stmt = $cnx->prepare($sql)) {
													if ($insert) {
														$stmt->bind_param("si", $content, $readOnly);
													} else {
														$stmt->bind_param("sii", $content, $readOnly, $id);
													}
													if (!$stmt->execute()) {
														echo $stmt->error;
													}
												...
												if ($query = $cnx->query($sql)) {
													...
													$sql = "SELECT id, content, readOnly FROM questions" . " LIMIT " . strval($lines_per_page) . " OFFSET " . strval($skip_lines);
											...
											><tbody
												><!--make a dynamic loop of values--><?php
													...
													><td
														><?php echo htmlspecialchars($row['readOnly']) ?></td
													></tr
												><tr
															...
															><div
																...
																><input type="hidden" name="id" value="<?php echo htmlspecialchars($row['id']) ?>"/><div
																	><div
																		...
																		></div
																	><div
																		><input type="text" name="content" value="<?php echo htmlspecialchars($row['content']) ?>"/></div
																	></div
																><div
																	><div
																		><label
																		...
																		></div
																	><div
																		><input type="text" name="readOnly" value="<?php echo htmlspecialchars($row['readOnly']) ?>"/></div
																	></div
																><div
																	><div
