							...
							><div
								...
								><div
									...
									><div
										...
										><div
											style="width:50%"
											><input type="button" value="Source participant" onclick="document.location = 'comedien.php?source=0'"/></div
										><div
											style="width:50%"
											><input type="button" value="Source client" onclick="document.location = 'comedien.php?source=1'"/></div
										></div
									><div
										class="vbox-a"
										><div
													...
													><div
														...
														><?php $sql = "SELECT e.id AS id, q.content AS content, e.likes AS likes, e.moderation AS moderation, e.selected AS selected, e.source AS source, NOW() AS date FROM questions AS q, evqust AS e WHERE e.id_event = " . $event . " AND e.id_question = q.id AND e.suppressed = 0 AND e.moderation = 1 AND e.source >= " . $source . " ORDER BY e.source DESC, e.likes DESC";
																if ($fetch = $cnx->query($sql)) {
																		...
																		logging($cnx, "ERROR", "error query - reason:" . $cnx->error, "", "comedien.php");
																}
																?></div
													></div
												><div
